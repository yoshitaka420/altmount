...
func appendQueueSourceFilter(conditions []string, args []any, source string) ([]string, []any) {
	const stremioMetadataPattern = `%"source":"stremio"%`

	switch source {
	case "stremio":
		conditions = append(conditions, "(download_id LIKE ? OR LOWER(category) = ? OR metadata LIKE ?)")
		args = append(args, "stremio:%", "stremio", stremioMetadataPattern)
	case "regular":
		conditions = append(conditions, "((download_id IS NULL OR download_id NOT LIKE ?) AND (category IS NULL OR LOWER(category) <> ?) AND (metadata IS NULL OR metadata NOT LIKE ?))")
		args = append(args, "stremio:%", "stremio", stremioMetadataPattern)
	}
