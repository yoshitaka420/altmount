...
func (s *Server) handleNzbCheck(c *fiber.Ctx) error {
	...
	fetchClient := httpclient.NewForExternal(cfg.Network, nzbCheckFetchTimeout)
	...
	for i := range items {
		...
		go func(i int) {
			...
			nzbData, err := fetchNzbForCheck(ctx, item.NzbURL, fetchClient)
			if err != nil {
				out.Error = err.Error()
				results[i] = out
				return
			...
			out.StreamBlocklistBlocked = res.StreamBlocklistBlocked
			if err != nil {
				out.Error = err.Error()
			}
			results[i] = out
...
func fetchNzbForCheck(ctx context.Context, nzbURL string, client *http.Client) ([]byte, error) {
