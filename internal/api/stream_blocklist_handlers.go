...
const (
	maxStreamBlocklistUploadBytes       = 256 * 1024 * 1024
	maxStreamBlocklistSourcesUploadByte = 5 * 1024 * 1024
	maxStreamBlocklistSourceItems       = 1000
)

type streamBlocklistSourcesResponse struct {
	Quorum         int                                     `json:"quorum"`
	LocalCount     int                                     `json:"localCount"`
	EffectiveCount int                                     `json:"effectiveCount"`
	TotalRows      int                                     `json:"totalRows"`
	Sources        []streamcheck.StreamBlocklistSourceInfo `json:"sources"`
}

type streamBlocklistImportResponse struct {
...
type streamBlocklistSourceMutateResponse struct {
	SourceID string `json:"sourceId,omitempty"`
	Message  string `json:"message,omitempty"`
	Removed  int64  `json:"removed,omitempty"`
}

type streamBlocklistSourcesImportResponse struct {
	Added   int `json:"added"`
	Skipped int `json:"skipped"`
	Invalid int `json:"invalid"`
}

func (s *Server) requireStreamBlocklist(c *fiber.Ctx) (*streamcheck.StreamBlocklistStore, bool) {
	if s.streamBlocklistStore == nil {
		_ = RespondServiceUnavailable(c, "Stream blocklist store not available", "")
		return nil, false
	}
	return s.streamBlocklistStore, true
}

func (s *Server) handleGetStreamBlocklist(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
...
func (s *Server) handleStreamBlocklistSources(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	quorum := 2
	if cfg != nil {
		quorum = cfg.GetStreamCheckStreamBlocklistQuorum()
	}
	return RespondSuccess(c, streamBlocklistSourcesResponse{
		Quorum:         quorum,
...
func (s *Server) handleStreamBlocklistImport(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	if c.FormValue("action") == "clear" {
		...
		if err != nil {
			return RespondInternalError(c, "Failed to clear stream blocklist list", err.Error())
		}
		return RespondSuccess(c, streamBlocklistImportResponse{Added: 0, Total: store.Count(ctx), Cleared: removed})
	}
	file, err := firstFormFile(c)
	if err != nil {
		return RespondBadRequest(c, "No file was uploaded", err.Error())
	}
	data, err := readUploadedFile(file, maxStreamBlocklistUploadBytes)
	if err != nil {
		return RespondBadRequest(c, "Failed to read stream blocklist import", err.Error())
	}
	body, err := decodeMaybeGzip(data, maxStreamBlocklistUploadBytes)
	if err != nil {
	...
	if c.FormValue("target") == "separate" {
		...
		return RespondSuccess(c, streamBlocklistImportResponse{Added: count, Total: store.Count(ctx), SourceID: sourceID})
	}
	before := store.LocalCount(ctx)
	if _, err := store.MergeIntoLocal(ctx, bytes.NewReader(body)); err != nil {
		return RespondBadRequest(c, "Failed to import stream blocklist list", err.Error())
	}
	after := store.LocalCount(ctx)
	return RespondSuccess(c, streamBlocklistImportResponse{Added: max(0, after-before), Total: store.Count(ctx)})
}

func (s *Server) handleStreamBlocklistExport(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	if err := store.ExportTo(c.Context(), gz, sourceIDs, c.Query("dedup") != "0"); err != nil {
		_ = gz.Close()
		return RespondInternalError(c, "Failed to export stream blocklist list", err.Error())
	}
...
func (s *Server) handleStreamBlocklistSourceAdd(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	message := ""
	if s.streamBlocklistRemote != nil {
		sources, _ := store.GetSources(c.Context())
		for _, source := range sources {
			if source.ID == sourceID {
				message, _ = s.streamBlocklistRemote.Refresh(c.Context(), source)
				break
			}
		}
	}
	return RespondSuccess(c, streamBlocklistSourceMutateResponse{SourceID: sourceID, Message: message})
}

func (s *Server) handleStreamBlocklistSourceUpdate(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	return RespondSuccess(c, streamBlocklistSourceMutateResponse{SourceID: id})
}

func (s *Server) handleStreamBlocklistSourceRemove(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	if c.FormValue("action") == "clear" {
		...
		return RespondSuccess(c, streamBlocklistSourceMutateResponse{SourceID: id, Removed: removed})
	}
	...
	return RespondSuccess(c, streamBlocklistSourceMutateResponse{SourceID: id, Removed: count})
}

func (s *Server) handleStreamBlocklistSourceRefresh(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	for _, source := range sources {
		if source.ID == id {
			message, err := s.streamBlocklistRemote.Refresh(c.Context(), source)
			if err != nil {
				return RespondBadRequest(c, "Failed to refresh stream blocklist source", err.Error())
			}
			return RespondSuccess(c, streamBlocklistSourceMutateResponse{SourceID: id, Message: message})
		}
	}
	return RespondBadRequest(c, "Unknown source", "")
}

func (s *Server) handleStreamBlocklistSourcesImport(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
	...
	if file, err := firstFormFile(c); err == nil {
		if file.Size > maxStreamBlocklistSourcesUploadByte {
			return RespondBadRequest(c, "File is too large", "")
		}
		data, err := readUploadedFile(file, maxStreamBlocklistSourcesUploadByte)
		if err != nil {
	...
	if len(content) > maxStreamBlocklistSourcesUploadByte {
	...
	if added > 0 && s.streamBlocklistRemote != nil {
		go func() {
			if err := s.streamBlocklistRemote.RefreshDue(context.Background()); err != nil {
				_ = err
			}
		}()
	}
	return RespondSuccess(c, streamBlocklistSourcesImportResponse{Added: added, Skipped: skipped, Invalid: invalid})
}

func (s *Server) handleStreamBlocklistSourcesExport(c *fiber.Ctx) error {
	store, ok := s.requireStreamBlocklist(c)
	if !ok {
		return nil
	}
...
func parseRemoteSourceSpecs(content, defaultTrust string, defaultRefresh int) ([]streamcheck.RemoteSourceSpec, int) {
	...
	for _, raw := range strings.Split(content, "\n") {
		if len(specs) >= maxStreamBlocklistSourceItems {
			break
		}
