import type {
	...
	User,
	WardenImportResponse,
	WardenSourceMutateResponse,
	WardenSourcesImportResponse,
	WardenSourcesResponse,
	WardenTrust,
} from "../types/api";
import type {
	ConfigResponse,
...
class APIClient {
	...
	async getWardenSources() {
		return this.request<WardenSourcesResponse>("/warden-sources");
	}

	async importWardenList(params: {
		file: File;
		...
		return this.request<WardenImportResponse>("/warden-import", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async exportWardenList(scope: "local" | "merged" = "merged") {
		return this.download(`/warden-export?scope=${encodeURIComponent(scope)}`);
	}

	async addWardenSource(params: {
		url: string;
		name?: string;
		trust?: WardenTrust;
		refreshHours?: number;
		...
		return this.request<WardenSourceMutateResponse>("/warden-source-add", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async updateWardenSource(params: {
		id: string;
		enabled?: boolean;
		trust?: WardenTrust;
		refreshHours?: number;
		...
		return this.request<WardenSourceMutateResponse>("/warden-source-update", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async removeWardenSource(id: string) {
		const formData = new FormData();
		formData.append("id", id);

		return this.request<WardenSourceMutateResponse>("/warden-source-remove", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async refreshWardenSource(id: string) {
		const formData = new FormData();
		formData.append("id", id);

		return this.request<WardenSourceMutateResponse>("/warden-source-refresh", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async importWardenSources(params: {
		text?: string;
		file?: File;
		trust?: WardenTrust;
		refreshHours?: number;
		...
		return this.request<WardenSourcesImportResponse>("/warden-sources-import", {
			method: "POST",
			body: formData,
			headers: {},
		});
	...
	async exportWardenSources() {
		return this.download("/warden-sources-export");
	}
